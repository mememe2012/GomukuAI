...
class Main:
    def __init__(self):
        ...
        self.theme_path = os.path.join(self.base_dir, "themes", "dark.json")
        self.page = "home"
        self.font_family = "HarmonyOS_Sans_SC"
        self.version = "0.0.1"
        self.url = "https://github.com/mememe2012/GomukuAI"
        self.sound = {
            "info1" : pg.mixer.Sound("music/info1.wav"),
        }
        self.hwnd = GetHWND()
    ...
    def loadConfig(self):
        ...
        CONSOLE.print("Available themes: ", self.theme_list, style="green")
        CONSOLE.print("Available languages: ", self.lang_list, style="green")
...
Seccessfully inited.
        ...
        while self.running:
                ...
                if self.page == "home":
                    ...
                    if event.type == pgui.UI_BUTTON_PRESSED:
                        ...
                        if event.ui_element == self.btn_save:
                            ...
                            self.config["lang"]["default"] = self.choice_language.selected_option[0]
            ...
            if self.page == "home":
                ...
                if self.setting_page == 1:
                    ...
                    self.screen.blit(pg.font.Font(self.fontpath, 20).render(self.tr("Theme").format(theme=self.config["theme"]["default"]),True,self.style["fontcolor"]),(20,100))
                    pg.draw.line(self.screen, self.style["fontcolor"], (20,170), (1004,170), 1)
                    self.screen.blit(pg.font.Font(self.fontpath, 20).render(self.tr("Language").format(lang=self.config["lang"]["default"]),True,self.style["fontcolor"]),(20,180))
    ...
    def controls(self):
        ...
        self.choice_theme = pgui.elements.UIDropDownMenu(
            ...
            relative_rect=pg.Rect(20, 130, 200, 30),
            manager=self.settingmanager,
        )

        self.choice_language = pgui.elements.UIDropDownMenu(
            options_list=self.lang_list,
            starting_option=self.config["lang"]["default"],
            relative_rect=pg.Rect(20, 210, 200, 30),
            manager=self.settingmanager,
        )

        self.btn_clean = pgui.elements.UIButton(
            relative_rect=pg.Rect(20, 285, 200, 40),
            text=self.tr("Clean Temp Files and Logs"),
            manager=self.settingmanager,
        )

        self.btn_openlog = pgui.elements.UIButton(
            relative_rect=pg.Rect(250, 285, 200, 40),
    ...
    class ChessBoard:
        ...
        def draw(self):
            self.parent.GaussianScreen(self.parent.screen,
                                       [self.x, self.y, self.size, self.size],
                                       radius=self.config["radius"],
                                       border_radius=self.config["border_radius"],
                                       mask_color=self.config["bgcolor"]
                                       )
            bias = 0
            for i in range(15):
                ...
                self.parent.screen.blit(
                    pg.font.Font(self.parent.fontpath, 20).render(
                                chr(ord("A") + i),
                                True, 
                                self.config["textcolor"]
                            ), 
                        (
                            self.x + self.sidelen + i * self.chesslength - self.chesslength // 4 + bias, 
                            self.y + self.sidelen // 4 - bias
                        )
                     )
                ...
                self.parent.screen.blit(
                    pg.font.Font(self.parent.fontpath, 20).render(
                        ...
                        (
                            self.x + self.sidelen // 4 - bias, 
                            self.y + self.sidelen + i * self.chesslength - self.chesslength // 4 + bias
                        )
                     )
